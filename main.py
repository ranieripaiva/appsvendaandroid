...
class MainApp(App):
    ...
    def carregar_infos_usuarios(self):
        try:
            ...
            requisicao = requests.get(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)

        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        requisicao = requests.get(link)
        requisicao_dic = requisicao.json()

        #print("olha o erro", requisicao_dic)
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade)==float):
            ...
            requests.post(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}",
                          data=info)
            ...
            requisicao = requests.get(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
            print(requisicao)
            total_vendas = float(requisicao.json())
            total_vendas += preco
            info = f'{{"total_vendas": "{total_vendas}"}}'
            requests.patch(f"https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
                           data=info)
            homepage = self.root.ids["homepage"]
            homepage.ids["label_total_vendas"].text = f"[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]"
            self.mudar_tela("homepage")
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
...
#https://aplicativovendashash-4a872-default-rtdb.firebaseio.com/
#Link do Google REST API para Autenticação: https://cloud.google.com/identity-platform/docs/use-rest-api
#token AIzaSyAJ4oOFZH6F6j02GYZwnv9FLrJ28Mmbr-4